...
| 그림과 짝지을 이름이 모호함 | **판정 불가** |

한 장에 구조가 여럿이면 그림은 **제 캡션하고만** 대조합니다. 장에 있는 아무 이름에나
맞춰보면 뒤바뀐 그림이 통과해 버리기 때문입니다. 캡션은 위치로 가리고, 그림 하나에
이름 하나씩 배타적으로 짝지으며, 가릴 수 없으면 판정하지 않습니다.
...
python -m chemcheck.web               # 브라우저 화면 (기본 127.0.0.1:8000)
```

웹 화면은 판정과 함께 **이름이 가리키는 구조**와 **그림에서 읽은 구조**를 나란히 놓고,
두 구조의 최대 공통 부분을 뺀 나머지 — 즉 다른 곳 — 을 칠해 보여줍니다.
InChIKey 문자열만으로는 어디가 왜 틀렸는지 사람이 알 수 없기 때문입니다.
인식 결과가 갈리면 후보를 전부 보여주고 고르게 하며, 임의로 하나를 택하지 않습니다.

화면은 표준 라이브러리만으로 뜨고 외부 CDN을 부르지 않습니다. 회선이 끊겨도 동작합니다.

구조 인식기(OCSR)를 설치하면 그림 판정이 켜집니다. 없으면 이름 추출까지만 동작하고 그림은 전부 판정 불가로 보고합니다.

이름은 PubChem으로 해석합니다. PubChem에 못 닿으면 `chemcheck/offline_names.json`의
최소 대체표로 넘어갑니다 (행사장 네트워크 대비). PubChem이 살아 있으면 언제나 PubChem이
우선이며, 대체표는 `python scripts/make_offline_names.py` 로 다시 만듭니다.
