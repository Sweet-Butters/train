...
class Prediction:
    ...
    confidence 는 **인식기마다 다른 자다.** 같은 0.9 가 같은 뜻이 아니다.
      ...
      - decimer: 디코더가 토큰마다 낸 softmax 확률의 평균 (decimer_confidence).
        MolScribe 의 0.8 과 견줄 수 없는 다른 자다. 이 값의 분포는 아직 실측하지
        않았다 - D 의 50장 결과에서 엔진별로 보고 문턱을 따로 정한다. 그 전에는
        이 값을 문턱에 태우지 않는다 (verdict.judge 의 max(confidence) 는 엔진을
        가리지 않으므로 거기서 decimer 를 빼야 한다).
...
def decimer_confidence(tokens_with_confidence) -> float:
    ...
    평균이다 - 최솟값이 아니다. 긴 SMILES 에서 토큰 하나가 흔들린 것과 짧은 SMILES
    전체가 흔들린 것을 구분하려면 분포를 봐야 하는데, 그 분포는 D 의 50장 결과에서
    본다. 그 전까지는 가장 단순한 요약을 둔다.
    """
